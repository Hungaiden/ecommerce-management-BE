...
export const getBookingsByEmail = async (req: Request, res: Response) => {
    try {
        const bookings = await tourBookingService.getBookingsByEmail(
            req.params.email
        );
        const response: ResponseListSuccess<typeof bookings> = {
            code: 200,
            message: bookings.length
                ? "Lấy danh sách booking theo email thành công"
                : "Không tìm thấy booking nào cho email này",
            data: {
                hits: bookings,
                pagination: {
        ...
        const response: ResponseFailure = {
            ...
            path: req.path,
            message: "Lỗi server khi lấy danh sách booking theo email",
            errors: [],
        };
